    ...
    const fetchData = async () => {
      try {
        ...
        if (res.success && res.data.length > 0) {
          ...
          setIsEditing(true); // Set editing mode if data exists
        } else {
          setIsFirstTime(true); // Mark as first time if no data exists
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setIsEditing(true); // Switch to editing mode after first save
